...
var tmp_global_seed: int = 123456789
...
func init_server(port: int, max_players: int, is_also_client: bool, nickname: String) -> void:
	...
	spawn_player(1, true, tmp_global_seed, is_also_client)
...
func _on_peer_connected(peer_id: int) -> void:
	...
	rpc_id(peer_id, "spawn_player", peer_id, true, tmp_global_seed)
	for id in players.keys():
		if id == peer_id:
			continue
		rpc_id(id, "spawn_player", peer_id, false)
		rpc_id(peer_id, "spawn_player", id, false, tmp_global_seed, players[id].is_also_client)
		
		if players.has(id) and players.has(peer_id):
	...
	if players.has(peer_id):
		rpc_id(peer_id, "client_request_nickname")
	else:
		rpc_id(peer_id, "client_request_nickname")

@rpc("authority", "call_local", "reliable")
func client_request_nickname() -> void:
	rpc_id(1, "server_sync_nickname", multiplayer.get_unique_id(), players[multiplayer.multiplayer_peer.get_unique_id()].nickname)
...
@rpc("authority", "call_local", "reliable")
func spawn_player(peer_id: int, is_local: bool, server_seed: int = 0, is_also_client: bool = true) -> void:
	if players.has(peer_id):
		return
	...
	if not is_also_client:
		player.visible = false
	if is_local:
		player.nickname = local_nickname
		player.global_seed = server_seed
	add_child(player)
